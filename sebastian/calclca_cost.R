...
lca_files      <- list.files(path = dir_lca, pattern = 'lca_REMIND_monetization_1daly_project.csv')
...
#xxx here are duplicates condense to total from category and end_categories
...
SE$region <- as.character(SE$region)
SE  <- SE %>% filter(region != "World")
SE  <- SE %>% filter(region != "")
...
lca_o3$value <- lca_o3$value *0
lca_o3$category <- 'POF'


lca_pmf$value <- lca_pmf$value *0
...
# country, end_cat totals, value reference for inequality paper
lca_remind_end_category_category_total <- lca_remind %>% 
  group_by(scenario, region, period, end_category) %>% 
  dplyr::summarise(value = sum(value_reference))
write.csv(lca_remind_end_category_category_total,paste0(dir_outtab,'environmental_indicators_end_cost.csv'))
...
lca_remind_cost_per_tec <- dplyr::left_join(lca_remind_cost_per_tec, SE)


lca_remind_cost_per_tec$tax <- ((lca_remind_cost_per_tec$value_reference+lca_remind_cost_per_tec$value)/10^9)/lca_remind_cost_per_tec$SE

#sum up over techs

write.csv(lca_remind_cost_per_tec,paste0(dir_outtab,'environmental_indicators_cost_per_tec.csv'))
if (do.plots) {
