...
#divded by the 2017 ration of mortality to DALYS weighted by the abolute deaths globally
u_DALY <- 3600000 / 25
...
names(DALY)[3] <- 'indirect_cost_per_daly'
...
results <- inner_join(results, DALY, by=c("iso.iso.y"="Region", 'year'='Year')) %>% mutate(indirect_daly_cost = daly_total * indirect_cost_per_daly)
